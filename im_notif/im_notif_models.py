...
class mail_notification(models.Model):
    ...
    def _message2im(self, cr, uid, message):
        ...
        mtype = {'email': _(u'Имэйл'),
                 'comment': _(u'Сэтгэгдэл'),
                 'notification': _(u'Автомат мэдэгдэл'),
        }.get(message.type, '')

        about = message.subject or message.record_name or 'UNDEFINED'
        about = u'[Тухай] %s' % about
        if url:
            about =  u'<a Холбоос="%s">%s</a>' % (url, about)
        im_text = [
            u'Сайн байна уу',
            u'<Холбоос="%s">_____[Миний зурвас]_____</a>' % inbox_url,
            u'%s [Хэнээс] %s' % (message.type, author),
            about,
        ]
