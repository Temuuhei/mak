...
class hse_safety_plan(osv.osv):
    ...
    _columns = {
        ...
        'year': fields.selection([('2012', '2012'),('2013', '2013'),('2014', '2014'),('2015', '2015'),('2016', '2016'),('2017', '2017'),('2018', '2018'),('2019', '2019'),('2020', '2020'),('2021', '2021'),('2022', '2022'),('', ''),('', '')], 'Year', required=True),
        ...
        'indicator_type': fields.related('indicator_id','type', type='selection',
            selection=[('leading','Leading indicators'),('lagging','Lagging indicators'),('training','Training'),('env','Env')],
            string='Indicator type', readonly=True, store=True),
        'frequency': fields.selection([('season_1','Season 1'),('season_2','Season 2'),('season_3','Season 3'),('season_4','Season 4')], 'Frequency', required=True),
        'is_count': fields.boolean('Is count'),
        'count': fields.integer('Count'),
        'percent': fields.integer('Percent %', group_operator='avg'),
        'actual': fields.char('Actual', size=4),
        'actual_percent': fields.function(_set_percent, type='integer', string='Actual percent %', readonly=True, group_operator='avg', store=True),
    }
...
class hse_nope_lti(osv.osv):
    ...
    def _total_man_hour(self, cr, uid, ids, name, args, context=None):
        ...
        hour = 0
        day = 0
        cr.execute("select max(h1.date),min(h1.date) from hse_nope_lti h1 where h1.date>=(select max(h2.date) from hse_nope_lti h2 where h2.man_hour='0' and h2.project_id=h1.project_id)");
        str_cr = cr.fetchone()
        print 'str_cr  \n',str_cr
    ...
    def _inc_man_hour(self, cr, uid, ids, name, args, context=None):
        ...
        for obj_this in self.browse(cr, uid, self.search(cr, uid, [])):
            lti_date = False
            for item in self.browse(cr, uid, self.search(cr, uid, [('project_id','=',obj_this.project_id.id), ('man_hour','=','0'), ('date','<=',obj_this.date)])):
                if not lti_date:
                    lti_date = item.date
                else:
            ...
            if lti_date:
                ...
                day = 0
                max_date = lti_date
                for item in self.browse(cr, uid, self.search(cr, uid, [('project_id','=',obj_this.project_id.id),('date','>=',lti_date),('date','<=',obj_this.date)])):
    ...
    _columns = {
        ...
        'total_man_hour': fields.function(_total_man_hour, type='integer', string='Total man hour', multi='lti',
            store={ 'hse.nope.lti': (lambda self, cr, uid, ids, c={}: ids, [], 30), }, group_operator='max'),
        'total_day': fields.function(_total_man_hour, type='integer', string='Since the date registered lost time injury', multi='lti',
            store={ 'hse.nope.lti': (lambda self, cr, uid, ids, c={}: ids, [], 30), }, group_operator='max'),
        'inc_total_day': fields.function(_inc_man_hour, type='char', string='Increase since the date registered lost time injury', multi='inc_lti',
...
class hse_nope_lti_line(osv.osv):
    ...
    def _man_hour(self, cr, uid, ids, name, args, context=None):
        ...
        m_hour = 0;
        obj = self.browse(cr,uid,ids)[0]
        pr_hr = self.pool.get('hse.project.man.hour').search(cr, uid, [('project_id','=',obj.nope_lti_id.project_id.id)])
...
class hse_safety_meeting(osv.osv):
    ...
    def _set_name(self, cr, uid, ids, name, args, context=None):
        ...
        conf_ids = self.pool.get('hse.code.config').search(cr, uid, [('project_id','=',obj.project_id.id),('model_id','=',my_id)])
        if conf_ids:
            num_name = self.pool.get('hse.code.config').browse(cr,uid,conf_ids)[0].name
            max_count = 0
            cr.execute('SELECT id FROM hse_safety_meeting where project_id = %s and EXTRACT(YEAR FROM date) = %s ',(obj.project_id.id, datetime.strptime(obj.date, '%Y-%m-%d').year))
    ...
    def _sum_count(self, cr, uid, ids, name, args, context=None):
        res = {}
        obj = self.browse(cr,uid,ids)[0]
        res[obj.id] = len(self.pool.get('hse.safety.meeting.line').search(cr, uid, [('safety_meeting_id','=',obj.id)]))
        return res
    ...
    _columns = {
        ...
        'month': fields.function(_set_date, type='integer', string='Month', multi='dates', readonly=True, store=True),
        'day': fields.function(_set_date, type='integer', string='Day', multi='dates', readonly=True, store=True),
        'state': fields.selection([('draft', 'Draft'),('done', 'Done')], 'State', readonly=True, track_visibility='onchange'),
        ...
        'project_id': fields.many2one('project.project','Project', required=True, states={'done':[('readonly',True)]}),
        'monitoring_user_id': fields.many2one('res.users','Monitoring user', required=True, states={'done':[('readonly',True)]}),
        ...
        'participants_count': fields.function(_sum_count,type='integer', string='Participants count', 
            store={ 'hse.safety.meeting': (lambda self, cr, uid, ids, c={}: ids, [], 20), 
                    'hse.safety.meeting.line': (_get_line, ['participant_id'], 20)}),
        'subject': fields.char('Safety meeting subject', required=True, size=150, states={'done':[('readonly',True)]}),
        'managing_employee_ids': fields.many2many('hr.employee', 'hse_safety_meeting_employee_rel', 'safety_meeting_id','employee_id', 'Managing employee', states={'done':[('readonly',True)]}),
        ...
        'attachment_ids': fields.many2many('ir.attachment', 'hse_safety_meeting_ir_attachments_rel','safety_meeting_id', 'attachment_id', 'Attachments', states={'done':[('readonly',True)]}),
    ...
    def import_participants(self, cr, uid, ids, context=None):
        ...
        relay_id = self.pool.get('work.schedule.relay').search(cr, uid, [('relay','=',obj.part.upper()),('project_id','in',hr_project)])
        hr_table_ids = self.pool.get('time.table.details').search(cr ,uid, [('employee_state','in',['work_at_night','work_at_day']),('project_id','in',hr_project),('relay','in',relay_id),('department_id','=',obj.department_id.id),('date','=',(datetime.strptime(obj.date, '%Y-%m-%d') - timedelta(days=1)).strftime('%Y-%m-%d')),('date','<=',(datetime.strptime(obj.date, '%Y-%m-%d') + timedelta(days=1)).strftime('%Y-%m-%d'))])
        hr_empolyees = []
...
class hse_workplace_ispection(osv.osv):
    ...
    def _set_name(self, cr, uid, ids, name, args, context=None):
        ...
        if conf_ids:
            num_name = self.pool.get('hse.code.config').browse(cr,uid,conf_ids)[0].name
            max_count = 0
            cr.execute('SELECT id FROM hse_workplace_ispection where project_id = %s and EXTRACT(YEAR FROM date) = %s ',(obj.project_id.id, datetime.strptime(obj.date, '%Y-%m-%d').year))
            obj_ids = map(lambda x: x[0], cr.fetchall())
    ...
    _columns = {
        ...
        'day': fields.function(_set_date, type='integer', string='Day', multi='dates', readonly=True, store=True),
        'name': fields.function(_set_name, type='char', string='Number', readonly=True, store=True),
        'state': fields.selection([('draft', 'Draft'),('sent_mail', 'Sent to mail'),('repaired', 'Repaired')], 'State', readonly=True),
        'project_id': fields.many2one('project.project','Project', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'made_place': fields.char('Made place', size=200, required=True, readonly=True, states={'draft':[('readonly',False)]}),

        'part': fields.selection([('a', 'A'),('b', 'B'),('c', 'C')], 'Part', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'department_id': fields.many2one('hr.department','Department', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'captian_id': fields.many2one('hr.employee', 'Captians', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        
        'member_ids': fields.many2many('hr.employee', 'hse_workplace_ispection_mem_employee_rel', 'wo_is_id','employee_id', 'Members', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'partner_ids': fields.many2many('hse.partner', 'hse_workplace_ispection_partner_rel', 'wo_is_id','partner_id', 'Responsible partner', readonly=True, states={'draft':[('readonly',False)]}),
        'attachment_ids': fields.many2many('ir.attachment', 'hse_workplace_ispection_ir_attachments_rel','worplace_id', 'attachment_id', 'Attachments', readonly=True, states={'draft':[('readonly',False),]}),
        'cor_act_attach_ids': fields.many2many('ir.attachment', 'hse_workplace_ispection_cor_act_attach_rel','worplace_id', 'attachment_id', 'Correct action image', readonly=True, states={'sent_mail':[('readonly',False),]}),
        
        'wo_is_line': fields.one2many('hse.workplace.ispection.line', 'workplace_is_id', 'Workplace ispection lines', readonly=False, states={'repaired':[('readonly',True)]}),
    ...
    def _get_mail(self, cr , uid, ids, obj, context=None):
        ...
        body=u'<p>Танд Ажлын Байрны Үзлэгээс <a href="'+unicode(base_url)+u'/web?db='+unicode(db_name)+u'#id='+str(obj.id)+u'&view_type=form&model=hse.workplace.ispection&action='+str(action_id)+u'"> '+unicode(obj.name)+u'</a> дугаартай хариу арга хэмжээ авах шаардлагатай дутагдал '
        if obj.state =='sent_mail':
            body += u'ирлээ.</p>'
        if obj.state =='repaired':
            body += u'засагдсан төлөвт орлоо.</p>'

        body += u'<table> <tr> <td>Хэлтэс</td><td style="font-weight: bold;">'+unicode(obj.department_id.name)+u'</td></tr><tr><td>Ээлж</td>'
        ...
        for item in obj.wo_is_line:
            ...
            if obj.state=='repaired':
                body += u'<td>'+unicode(item.corrective_action_taken)+u'</td><td>'+unicode(item.taken_employee_id.name)+'</td><td>'+unicode(item.repair_date)+'</td>'
            body += u'</tr>'
        body += u'</table>'
        mail_text=''
        if obj.mail_text:
            mail_text=obj.mail_text
        self.send_emails(cr, uid, ids, u'Ажлын байрны үзлэг '+unicode(obj.name), unicode(body)+u'<br/>'+unicode(mail_text))
        sent_mail_users = ''
        for item in obj.mail_line:
        ...
        self.pool.get('hse.workplace.ispection').message_post(cr, uid, [obj.id], body=unicode(message_post+u':<br/>Дараах хүмүүст майл илгээгдэв:<br/>'+sent_mail_users), type='notification', subtype=None, parent_id=False, context=context)
    ...
    def mail_sent(self, cr, uid, ids, context=None):
        ...
        if obj.state =='draft':
            ...
            if obj.state =='sent_mail':
                ...
                taken_employee_id = self.pool.get('hr.employee').browse(cr, uid, self.pool.get('hr.employee').search(cr, uid, [('user_id','=',uid)])).id
                self.pool.get('hse.workplace.ispection.line').write(cr, uid,self.pool.get('hse.workplace.ispection.line').search(cr, uid, [('workplace_is_id','=',obj.id),('is_repaired','=',False)]), {'is_repaired': True, 'repair_date': datetime.now().strftime('%Y-%m-%d'), 'taken_employee_id':taken_employee_id})
    ...
    def action_to_sent_mail(self, cr, uid, ids, context=None):
        ...
        if obj.wo_is_line:
            ...
            if not obj.mail_line:
                ...
                for item in members:
                    user_obj = self._get_email_employee(cr, uid, ids, self.pool.get('hr.employee').browse(cr, uid, item), user_mails, context)
                    if user_obj:
                ...
                for item in obj.wo_is_line:
                    members = [x.id for x in item.employee_ids]
                    for line in members:
                        user_obj = self._get_email_employee(cr, uid, ids, self.pool.get('hr.employee').browse(cr, uid, line), user_mails, context)
                        if user_obj:
                            user_mails.append(user_obj['email'])
                    members = [x.id for x in item.partner_ids]
                    for line in members:
            ...
            view_id = self.pool.get('ir.ui.view').search(cr, uid, [('model', '=', 'hse.workplace.ispection'), ('name', '=', 'hse.workplace.ispection.mail.form')])
            return {
...
class hse_workplace_ispection_mail_line(osv.osv):
    ...
    _columns = {
        'workplace_is_id': fields.many2one('hse.workplace.ispection','Workplace ID', required=True, ondelete='cascade'),
...
class hse_workplace_ispection_line(osv.osv):
    ...
    def action_to_repaired(self, cr, uid, ids, context=None):
        ...
        if obj.corrective_action_taken:
            if len(self.pool.get('hse.workplace.ispection.line').search(cr, uid, [('workplace_is_id','=',obj.workplace_is_id.id),('is_repaired','=',False)]))==1:
                view_id = self.pool.get('ir.ui.view').search(cr, uid, [('model', '=', 'hse.workplace.ispection'), ('name', '=', 'hse.workplace.ispection.mail.form')])
                return {
                    'name':_("Дараах хүмүүст майл илгээж мэдэгдэх"),
                    'view_mode': 'form',
                ...
                taken_employee_id = self.pool.get('hr.employee').browse(cr, uid, self.pool.get('hr.employee').search(cr, uid, [('user_id','=',uid)])).id
                self.write(cr, uid, ids, {'is_repaired': True, 'repair_date': datetime.now().strftime('%Y-%m-%d'), 'taken_employee_id':taken_employee_id})
        else:
    ...
    _columns = {
        ...
        'hazard_rating': fields.selection([('a', 'A'),('b', 'B'),('c', 'C')], 'Hazard rating', required=True,  readonly=True, states={'draft':[('readonly',False)]}),
        'failure_and_hazard': fields.char('Failure and hazard', size=300, required=True,  readonly=True, states={'draft':[('readonly',False)]}),
        'required_corr_action': fields.char('Required corrective action', size=400, required=True,  readonly=True, states={'draft':[('readonly',False)]}),
        'responsible': fields.reference('Responsible', selection=[('hr.employee','Employee'),('hse.partner', 'Partner')]),
        'employee_ids': fields.many2many('hr.employee', 'hse_workplace_ispection_line_employee_rel', 'wo_is_id','employee_id', 'Responsible employee', required=False, readonly=True, states={'draft':[('readonly',False)]}),
        'partner_ids': fields.many2many('hse.partner', 'hse_workplace_ispection_line_partner_rel', 'wo_is_id','partner_id', 'Responsible partner', required=False, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        'corrective_action_taken': fields.char('Corrective action taken', size=200, readonly=True, states={'sent_mail':[('readonly',False)]}),
...
class hse_hazard_report(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        if vals['auto_number'] == True:
            obj_ids = self.pool.get('hse.hazard.report').search(cr, uid, [('project_id','=',vals['project_id']),('year','=',datetime.strptime(vals['datetime'], '%Y-%m-%d %H:%M:%S').year),('auto_number','=',True)])
            max_count = 0
    ...
    def write(self, cr, uid, ids, vals,context=None):
        ...
        if obj.auto_number and not obj.name:
            date_object = datetime.strptime(obj.datetime, '%Y-%m-%d %H:%M:%S')
            obj_ids = self.pool.get('hse.hazard.report').search(cr, uid, [('project_id','=',obj.project_id.id),('year','=',date_object.year),('auto_number','=',True),('id','!=',obj.id)])
            max_count = 0
            for item in self.pool.get('hse.hazard.report').browse(cr, uid, obj_ids, context=context):
                s = item.name
                if s and int(s[1: len(s)]) > max_count:
    ...
    _columns = {
        ...
        'project_id': fields.many2one('project.project','Project', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'location_id': fields.many2one('hse.location','Location', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'hazard_type': fields.selection([('minor', 'Minor'),('medium', 'Medium'),('seriuos', 'Seriuos')], 'Hazard type', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'notify_emp_id': fields.reference('Notify employee', selection=[('hr.employee','Employee'),('hse.partner', 'Partner')], required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'hazard_identification': fields.text('Hazard identification', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'corrective_action_to_be_taken': fields.text('Corrective action to be taken', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'corrective_action_taken': fields.text('Corrective action taken', readonly=True, states={'sent_mail':[('readonly',False),('required',True)]}, required=False),
        'taken_datetime': fields.datetime('Taken dateime', readonly=True),
        'responsible' : fields.reference('Responsible', selection=[('hr.employee','Employee'),('hse.partner', 'Partner')], required=True, readonly=True, states={'draft':[('readonly',False)]}),
    ...
    def _get_mail(self, cr , uid, ids, obj, context=None):
        ...
        body = u'<p style="font-weight: bold;"> Дараах <a href="'+unicode(base_url)+u'/web?db='+unicode(db_name)+u'#id='+str(obj.id)+u'&view_type=form&model=hse.hazard.report&action='+str(action_id)+u'">Аюулыг мэдээлэх хуудас '+unicode(obj.name)+u'</a> '
        if obj.state =='sent_mail':
            body += u'ирлээ.</p>'
        if obj.state =='repaired':
            body += u'засагдсан төлөвт орлоо.</p>'
        ...
        body += u'<td style="font-weight: bold;">'+unicode(obj.project_id.name)+u'</td></tr><tr><td>Аюулын төрөл</td>'
        ...
        if obj.mail_text:
            mail_text=obj.mail_text
        self.send_emails(cr, uid, ids, u'Аюулыг мэдээлэх хуудас '+unicode(obj.name), body+u'<br/>'+unicode(mail_text), context=context)
        sent_mail_users = ''
        for item in obj.mail_line:
            sent_mail_users += item.mail+'<br/>'
        ...
        self.pool.get('hse.hazard.report').message_post(cr, uid, [obj.id], body=unicode(message_post+u':<br/>Дараах хүмүүст майл илгээгдэв:<br/>'+sent_mail_users),type='notification', subtype=None, parent_id=False, context=context)
    ...
    def mail_sent(self, cr, uid, ids, context=None):
        ...
        if obj.state =='draft':
            ...
            if obj.state =='sent_mail':
                taken_employee_id = self.pool.get('hr.employee').browse(cr, uid, self.pool.get('hr.employee').search(cr, uid, [('user_id','=',uid)])).id
                self.write(cr, uid, ids, {'state': 'repaired' , 'taken_datetime': datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 'taken_employee_id':taken_employee_id})
    ...
    def action_to_sent_mail(self, cr, uid, ids, context=None):
        ...
        view_id = self.pool.get('ir.ui.view').search(cr, uid, [('model', '=', 'hse.hazard.report'), ('name', '=', 'hse.hazard.report.mail.form')])
        return {
            'name':_("Дараах хүмүүст майл илгээж мэдэгдэх"),
            'view_mode': 'form',
            'view_id': view_id,
...
class hse_risk_assessment(osv.osv):
    ...
    def _set_name(self, cr, uid, ids, name, args, context=None):
        ...
        if conf_ids:
            num_name = self.pool.get('hse.code.config').browse(cr,uid,conf_ids)[0].name
            max_count = 0
            cr.execute('SELECT id FROM hse_risk_assessment where project_id = %s and EXTRACT(YEAR FROM datetime) = %s ',(obj.project_id.id, datetime.strptime(obj.datetime, '%Y-%m-%d %H:%M:%S').year))
            obj_ids = map(lambda x: x[0], cr.fetchall())
    ...
    _columns = {
        ...
        'name': fields.function(_set_name, type='char', string='Number', readonly=True, store=True),
        'project_id': fields.many2one('project.project','Project', required=True, states={'done':[('readonly',True)]}),
        'location_id': fields.many2one('hse.location','Location', required=True, states={'done':[('readonly',True)]}),
        'made_work': fields.char('Made Work', required=True, size=200, states={'done':[('readonly',True)]}),
        'monitoring_user_id': fields.many2one('hr.employee','Monitoring user', required=True, states={'done':[('readonly',True)]}),
        'captian_user_id': fields.many2one('hr.employee','Captian', required=True, states={'done':[('readonly',True)]}),
        'member_ids': fields.many2many('hr.employee', 'hse_risk_assessment_mem_employee_rel', 'wo_is_id','employee_id', 'Members', states={'done':[('readonly',True)]}),
        'risk_assessment_table': fields.one2many('hse.risk.assessment.table', 'risk_assessment_id', 'Workplace ispection lines', states={'done':[('readonly',True)]}),
        'attachment_ids': fields.many2many('ir.attachment', 'hse_risk_assessment_ir_attachments_rel','assessment_id', 'attachment_id', 'Attachments', readonly=True, states={'draft':[('readonly',False)]}),
    }
    def action_to_done(self, cr, uid, ids, context=None):
        obj = self.browse(cr,uid,ids,context)[0]
        self.write(cr, uid, ids, {'state': 'done'})
...
class hse_risk_assessment_table(osv.osv):
    ...
    _columns = {
        ...
        'employee_ids': fields.many2many('hr.employee', 'hse_workplace_ispection_line_employee_rel', 'wo_is_id','employee_id', 'Responsible employee'),
        'partner_ids': fields.many2many('hse.partner', 'hse_workplace_ispection_line_partner_rel', 'wo_is_id','partner_id', 'Responsible partner'),
...
class hse_injury_entry(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'datetime' in vals or 'project_id' in vals or 'accident_type' in vals:
            ...
            if conf_ids:
                ...
                obj_ids = self.pool.get('hse.injury.entry').search(cr, uid, [('id','!=',obj.id),('project_id','=',obj.project_id.id),('year','=',obj.year),('accident_type','=',obj.accident_type.id)])
    ...
    _columns = {
        ...
        'technic_ids': fields.many2many('technic', 'hse_injury_entry_technic_rel','injury_id', 'technic_id', 'Technic'),
        ...
        'project_id': fields.many2one('project.project','Project', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'department_id': fields.many2one('hr.department','Department', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'project_manager_id': fields.many2one('hr.employee','Project manager',  required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'dep_manager_id': fields.many2one('hr.employee','Department manager',  readonly=True, states={'draft':[('readonly',False)]}),
        'general_master_id': fields.many2one('hr.employee','General master',  readonly=True, states={'draft':[('readonly',False)]}),
        'master_id': fields.many2one('hr.employee','Master', readonly=True, states={'draft':[('readonly',False)]}),
        
        'accident_type': fields.many2one('hse.accident.type', 'Accident type', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'value': fields.related('accident_type','value', type='char', relation='hse.accident.cause', store=True, string='Value'),

        'location_accident': fields.char('The location of the accident', size=100, required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'consequence_id': fields.many2one('hse.consequence', 'Potential significance', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'likelihood_id': fields.many2one('hse.likelihood', 'Likelihood reoccure', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        if 'project_id' in vals or 'accident_type' in vals:
            ...
            if conf_ids:
                ...
                if obj.accident_type.value =='FIRST_AID':
                    ...
                    num_name+=u'ӨМЧ-'
                max_count = 0
                cr.execute('SELECT id FROM hse_injury_entry where project_id = %s and EXTRACT(YEAR FROM datetime) = %s and accident_type=%s',(obj.project_id.id, datetime.strptime(obj.datetime, '%Y-%m-%d %H:%M:%S').year, obj.accident_type.id))
                obj_ids = map(lambda x: x[0], cr.fetchall())
                for item in self.pool.get('hse.injury.entry').browse(cr, uid, obj_ids, context=context):
                    s = item.name
                    if s and int(s[len(num_name): len(s)]) > max_count:
    ...
    def mail_sent(self, cr, uid, ids, context=None):
        ...
        if obj.state =='draft':
            ...
            taken_employee_id = self.pool.get('hr.employee').browse(cr, uid, self.pool.get('hr.employee').search(cr, uid, [('user_id','=',uid)])).id
            self.pool.get('hse.injury.corrective.action.line').write(cr, uid,self.pool.get('hse.injury.corrective.action.line').search(cr, uid, [('injury_id','=',obj.id),('is_taken','=',False)]), {'is_taken': True, 'taken_date': datetime.now().strftime('%Y-%m-%d'), 'taken_employee_id':taken_employee_id})
        ...
        subject = ''
        body = u'Дараах <a href="'+unicode(base_url)+u'/web?db='+unicode(db_name)+u'#id='+str(obj.id)+u'&view_type=form&model=hse.injury.entry&action='+str(action_id)+u'">Осол тохиолдол '+unicode(obj.name)+u'</a> '
        ...
        if obj.state =='sent_mail':
            ...
            for item in obj.corrective_action_line:
                ...
                body += u'<tr><td>'+item.corrective_action_what+u'</td><td>'+members_name+u'</td>'
                body += u'<td>'+item.how+u'</td><td>'+item.when_start+' - '+item.when_end+'</td>'
                body += u'<td>'+unicode(item.corrective_action_taken)+u'</td><td>'+unicode(item.taken_employee_id.name)+'</td><td>'+unicode(item.taken_date)+'</td>'
                body += u'</tr>'
            body += u'</table>'
        ...
        if obj.mail_text:
            mail_text=obj.mail_text
        self.send_emails(cr, uid, ids, u'Осол тохиолдол '+unicode(obj.name)+subject, body+u'<br/>'+unicode(mail_text))
        sent_mail_users = ''
        for item in obj.mail_line:
            sent_mail_users += item.mail+'<br/>'
        ...
        self.pool.get('hse.injury.entry').message_post(cr, uid, [obj.id], body=unicode(message_post+u':<br/>Дараах хүмүүст майл илгээгдэв:<br/>'+sent_mail_users), type='notification', subtype=None, parent_id=False, context=context)
        
        return True

    def action_to_cor_act_closed(self, cr, uid, ids, context=None):
        obj = self.browse(cr,uid,ids,context)[0]
        view_id = self.pool.get('ir.ui.view').search(cr, uid, [('model', '=', 'hse.injury.entry'), ('name', '=', 'hse.injury.entry.mail.form')])
        return {
            'name':_("Дараах хүмүүст майл илгээж мэдэгдэх"),
            'view_mode': 'form',
            'view_id': view_id,
    ...
    def action_to_closed(self, cr, uid, ids, context=None):
        ...
        if obj.corrective_action_line:
            ...
            emp_obj = self.pool.get('hr.employee').browse(cr, uid, self.pool.get('hr.employee').search(cr, uid, [('user_id','=',obj.project_manager_id.id)]))
    ...
    def import_cause(self, cr, uid, ids, context=None):
        ...
        for item in cause_ids:
            data = { 'injury_id': obj.id,
                     ...
                     'type': self.pool.get('hse.accident.factor').browse(cr, uid, [self.pool.get('hse.accident.cause').browse(cr, uid, item).factor_id.id]).type
                     }
...
class hse_injury_accident_line(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        obj = self.browse(cr,uid,ids,context)[0]
        if 'check' in vals:
            factor_ids = self.pool.get('hse.injury.factor.line').search(cr, uid, [('injury_id','=',obj.injury_id.id), ('factor_id','=',obj.factor_id.id)])
            if not obj.check:
                if not factor_ids:
                    data = {
                    ...
                    for item in self.browse(cr, uid, self.search(cr, uid, [('injury_id','=',obj.injury_id.id), ('factor_id','=',obj.factor_id.id), ('check','=',True)])):
                        note += item.accident_cause_id.name+'\n'
                    self.pool.get('hse.injury.factor.line').write(cr, uid, factor_ids, {'notes': note}, context=context)
            else:
                lines_ids = self.search(cr, uid, [('injury_id','=',obj.injury_id.id), ('factor_id','=',obj.factor_id.id), ('check','=',True), ('id','!=',obj.id)])
                if lines_ids:
                    note = ''
                    for item in self.browse(cr, uid, self.search(cr, uid, [('injury_id','=',obj.injury_id.id), ('factor_id','=',obj.factor_id.id), ('check','=',True), ('id','!=',obj.id)])):
                        note += item.accident_cause_id.name+'\n'
                    self.pool.get('hse.injury.factor.line').write(cr, uid, factor_ids, {'notes': note}, context=context)
                else:
    ...
    _columns = {
        ...
        'sequence': fields.related('accident_cause_id','sequence', type='integer', string='Sequence', relation='hse.accident.cause', store=True, readonly=True),
        'factor_id': fields.related('accident_cause_id','factor_id', type='many2one', string='Factor', relation='hse.accident.factor', store=True, readonly=True),
...
class hse_injury_factor_line(osv.osv):
    ...
    _columns = {
        ...
        'state': fields.related('injury_id','state', type='char', string='State', relation='hse.injury.entry'),
        'type': fields.related('factor_id','type', type='char', string='Type', relation='hse.accident.factor', readonly=True),
    }
...
class hse_injury_corrective_action_line(osv.osv):
    ...
    def action_to_taken(self, cr, uid, ids, context=None):
        obj = self.browse(cr,uid,ids,context)[0]
        if obj.corrective_action_taken:
            if len(self.pool.get('hse.injury.corrective.action.line').search(cr, uid, [('injury_id','=',obj.injury_id.id),('is_taken','=',False)]))==1:
                view_id = self.pool.get('ir.ui.view').search(cr, uid, [('model', '=', 'hse.injury.entry'), ('name', '=', 'hse.injury.entry.mail.form')])
                return {
                ...
                taken_employee_id = self.pool.get('hr.employee').browse(cr, uid, self.pool.get('hr.employee').search(cr, uid, [('user_id','=',uid)])).id
                self.write(cr, uid, ids, {'is_taken': True, 'taken_date': datetime.now().strftime('%Y-%m-%d'), 'taken_employee_id':taken_employee_id})
        else:
            raise osv.except_osv(_('Error'), _('Corrective action taken is empty'))
        return True
    ...
    _columns = {
        ...
        'corrective_action_what': fields.char('Corrective action what?', size=350,required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'who': fields.many2one('hr.employee', string='Who?', readonly=True, states={'draft':[('readonly',False)]}),
        'employee_ids': fields.many2many('hr.employee', 'hse_injury_corrective_action_line_employee_rel', 'injury_id','employee_id', 'Who employee', required=False, readonly=True, states={'draft':[('readonly',False)]}),
        'partner_ids': fields.many2many('hse.partner', 'hse_injury_corrective_action_line_partner_rel', 'injury_id','partner_id', 'Who partner', required=False, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        'corrective_action_taken': fields.text('Corrective action taken', readonly=True, states={'closed':[('readonly',False)]}),
        'taken_employee_id': fields.many2one('hr.employee','Taken employee', readonly=True),
...
class hse_injury_audit_line(osv.osv):
    ...
    _columns = {
        ...
        'job_id': fields.related('employee_id','job_id', type='many2one', string='Job', relation='hr.job', store=True, readonly=True),
    }   
class hse_injury_audit_conclusion_line(osv.osv):
    ...
    _columns = {
        'injury_id': fields.many2one('hse.injury.entry','Accident ID', required=True, ondelete='cascade'),
        'state': fields.related('injury_id','state', type='char', string='State', readonly=True),
        'employee_id': fields.many2one('hr.employee','Name', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'job_id': fields.related('employee_id','job_id', type='many2one', string='Job', relation='hr.job', store=True, readonly=True),
...
class hse_risk_rating(osv.osv):
    ...
    _columns = {
        ...
        'assessment_type': fields.selection([('low_risky','Low risky'),('risky','Risky'),('high_risky','High risky')],'Assessment type', required=True),
...
class hse_accident_factor(osv.osv):
    ...
    _columns = {
        ...
        'type': fields.selection([('immediate_cause','Immediate cause'),('base_cause','Base cause')], 'Type', required=True),
