            ...
            if (values.get('start_date') or values.get('start_datetime')) and values.get('active', True):
                for the_id in real_ids + new_ids:
                    ...
                    if mail_to_ids:
                        ...
                        if self.pool['calendar.attendee']._send_mail_to_attendees(cr, uid, mail_to_ids, template_xmlid='mak_reminder.calendar_template_reminder_changedate', email_from=current_user.email, context=context):
